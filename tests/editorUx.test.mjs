...
assert.match(html, /data-rich-command="tab"/);
assert.match(html, /data-rich-command="untab"/);
assert.equal((html.match(/data-rich-command="tab"/g) || []).length, 3, "Tab button must exist on all three rich toolbars");
assert.equal((html.match(/data-rich-command="untab"/g) || []).length, 3, "Shift+Tab button must exist on all three rich toolbars");
assert.match(html, /title="Tab \(sejajarkan ke numbering\)"/);
assert.match(html, /lucide-indent-increase/);
assert.match(html, /lucide-indent-decrease/);
assert.match(html, /event\.key === "Tab"/);
assert.match(html, /event\.shiftKey \? "out" : "in"/);
assert.match(html, /buildIndentCommandEdits\(event\.target\.value/);
