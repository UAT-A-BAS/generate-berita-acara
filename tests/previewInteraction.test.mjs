...
assert.match(html, /previewHighlightedInput:\s*null/);
assert.match(html, /clearTimeout\(appState\.previewHighlightTimer\);\s*if \(appState\.previewHighlightedInput\)/);
assert.match(html, /appState\.previewHighlightedInput\.classList\.remove\("preview-input-hit"\)/);
assert.match(html, /appState\.previewHighlightedInput = input/);
assert.match(html, /if \(appState\.previewHighlightedInput !== input\) return/);
assert.match(html, /\.preview-input-hit\s*\{[\s\S]*animation:\s*annotation-fade 2s ease-out forwards/);
...
assert.match(html, /data-preview-field="pic"[\s\S]*vertical-align:\s*middle\s*!important/);
assert.match(html, /const verticalAlign = key === "pic" \|\| \(isMerged && key === "result"\) \? "middle" : "top"/g);
